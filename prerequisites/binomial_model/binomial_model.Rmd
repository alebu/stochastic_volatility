...
It is easy to see that $\mathcal{F}_t$ is the **generated filtration** of $(Z_T)$.
That is, $\mathcal{F}_t$ contains information around changes in stock prices up 
to and including time $t$. So, we have:
...
that we will use to derive the price of the contingent claims (dee the 
replication argument for pricing).

Note also that how we defined the measurability of the portfolio strategy makes 
sense: we make decisions on what portfolio to hold durign the transition 
...
Check this last definition, shouldn't it be $x_{t - 1}(1 + r)$?

Also, to simplify the treatment, let's define the value of the contingent claim
$\phi$ at time t as $V_t^\phi$. We can imagine this as being the value process of
a portfolio strategy composed only by the contingent claim $\phi$.
...
$h_t$ such that $h_T - \phi(S_T)$ has the same value, in T, in every possible state
...
contingent claim. Furthermore, we'll be able to express that prices as the 
expectation under a particular probability measure, $\mathbb{Q}$. Under this
measure:
...
growth of cash. Obviously this different from the real world, where investors
require a higher growth for stock in order to assume the risk associated with it.

# The one-period market

In the one-period market model, time takes two values: 0 and 1. The market model
that we defined for the general case becomes a lot simpler. Since there is only
one time transition, we don't need a stochastic process to model portfolio 
strategies. Instead, we can model them as $h = (x, y) \in \mathbb{R}^2$, where 
$x$ and $y$ are the amounts of cash and stock that we hold during the transition,
that is, that we compose at time zero. 
...
$$
S_1 =  \begin{cases}
      su & \text{with probability}\\
      sd & \text{with probability}
    \end{cases}  
$$
...
the value of this portfolio at time zero must be equal to the value (and hence, 
...
$$
$$
= \frac{1}{(1 + r)} \frac{\phi(sd)u - \phi(su)d }{u - d} + \frac{\phi(su) - \phi(sd) }{u - d} = \\
$$
$$
...
This is known as the $risk-neutral pricing formula$. As we can see, the price of
the derivative does not depend on the actual probabilities. We can note that even
